  ...
  const testApiConnection = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        setApiStatus({
          status: 'success',
          message: data.message || '模拟API连接成功'
        });
      } else {
        setApiStatus({
          status: 'warning',
          message: `${data.message || '连接受限'} - ${data.solution || '使用模拟数据'}`
        });
      }
    } catch (err: any) {
      setApiStatus({
        status: 'error',
        message: `连接错误: ${err.message || '未知错误'} - 使用模拟数据继续运行`
      });
    } finally {
          ...
          >
            <Group justify="space-between">
              ...
              <Group>
                ...
                >
                  {isTestingApi ? "测试中..." : apiStatus?.status === 'success' ? "API连接正常" : apiStatus?.status === 'warning' ? "使用模拟数据" : "测试API连接"}
                </Button>
                <Button 
                  variant="subtle" 
                  color="gray" 
            ...
            {apiStatus && (
              <Alert 
                ...
                title={apiStatus.status === 'success' ? '连接成功' : apiStatus.status === 'warning' ? '使用模拟数据' : '连接失败'} 
                withCloseButton 
