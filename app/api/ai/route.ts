...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ 
      status: 'error', 
      message: '无法连接到Google API（网络限制）', 
      error: '由于网络限制，无法访问Google API服务',
      solution: '应用将使用模拟数据继续运行',
      networkInfo: {
        googleAccessible: false,
        baiduAccessible: true
...
export async function POST(request: NextRequest) {
  try {
    const { prompt, model = 'gemini-2.0-flash', useSimulation = false } = await request.json();

    if (!prompt) {
      return NextResponse.json({ error: '缺少必要的prompt参数' }, { status: 400 });
    }

    // 如果请求指定使用模拟数据或者我们知道API无法访问，直接返回模拟响应
    if (useSimulation) {
    ...
    try {
      console.log('尝试使用@google/generative-ai库连接Google API...');
      
      // 使用@google/generative-ai库
...
How can I help you learn more about specific aspects of the MindHive Network?`;
  ...
  return NextResponse.json({
    ...
    networkActivity,
    metadata: {
      model: 'simulated-gemini-2.0-flash',
